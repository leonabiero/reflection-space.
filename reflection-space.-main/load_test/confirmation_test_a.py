...
practitioner does during a session:
    ...
    7. Begin Reflection                      (runs the real live historical-context
                                               retrieval path, then a FAKE/mocked
                                               reflection generation -- $0 cost) --
                                               users who finalized
...
def simulate_one_user(user_index, run_id, pause_range, timeout, metrics):
    ...
    if random.random() < 0.7:
        ...
        # 7. Begin Reflection -- use the same live historical-context
        # retrieval path as pages/reflection_space.py. There is no cache
        # lookup anymore, so this measures the real retrieval latency.
        r = common.timed_with_timeout(
