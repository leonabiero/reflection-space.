...
render_nav(T, page_name="reflection_space")
render_identity_footer(T)

with error_boundary(
    "reflection_space", T=T, user_name=user_name, user_role=user_role,
):
    ...
    if active_context is not None:
        ...
        with col1:
            if st.button(
                ...
                with error_boundary(
                    "reflection_space (generate reflection)",
                    T=T,
                    user_name=user_name,
                    user_role=user_role,
                    context={"case_ref": getattr(ctx, "case_ref", "")},
    ...
    for case_ref in sorted(by_case.keys(), key=lambda s: s.lower()):
        ...
        with st.expander(f"{folder_icon} {case_ref} — {len(case_drafts)}"):
            ...
            if st.button(T["begin_reflection"], key=f"begin_{case_ref}", disabled=not selected):
                ...
                # Historical context is always retrieved live. This is the
                # only retrieval path now that the former background prefetch
